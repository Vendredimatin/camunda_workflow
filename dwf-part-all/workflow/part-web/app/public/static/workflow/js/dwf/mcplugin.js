...
(function($, window, document, undefined) {
  var pluginName = "mainContentPlugin";
	...
	MContentPlugin.prototype.createMainContentArea = function(parent) {
		...
		// tabContent.className = "tab-content tabcontents";
		// tabContent.id = "tabcontents"+this.options.pid;
	};
    ...
    var that = this;
    var s;
    console.log("that.options.pid",that.options.pid);
		$(".main-nav-tabs"+this.options.pid).on("click", "a", function(e) {
			e.preventDefault();
			$(this).tab('show');
      s = $(this).attr('href');
      ...
      // // s = $(this).attr('data');
			// // var pp = s.substring(5);
			// console.log("pp",pp);
			// console.log("map[pp]",map[pp]);
			// if (map[pp] != null) {
			// 	var dirty = map[pp].getDirty();
			// 	if (dirty && !(map[pp].wfprocess instanceof  WfProcessInstance)) {
	...
	MContentPlugin.prototype.clearPropertySheet = function(e) {
		// basic propertyconfirmInfoDialog
    ...
    console.log("mcplugin savebutton");
		var that = this;
    var id = this.options.pid;// #tab_000002A8AS
    if (map[id] != null && map[id].getDirty()) {
      // that.options.parent.saveObjects(id, closetab, parent, anchor);
      that.options.parent.saveObjects(id);
    }
		// $(".main-nav-tabs"+this.options.pid).children('li').each(function(entry) {
		// 	var anchor = $(this).children("a")[0];
		// 	var id = $(anchor).attr('href').substring(5);// #tab_000002A8AS
	...
	MContentPlugin.prototype.cloaseTab = function(id, anchors) {
		...
		this.currentProcID = -1;
		$(".main-nav-tabs"+this.options.pid+" li").children('a').last().click();

		curPrefer.removeId(id); // remove tag by id
		this.savePreferChange();// save tag after remove id
	};

  MContentPlugin.prototype.addNewTab = function(myTabTitle, myTabID, ownerID, type) {
    var tabcontent = document.createElement("DIV");
    tabcontent.setAttribute('role', 'tabpanel');
    tabcontent.className = "tab-pane0";
	...
	MContentPlugin.prototype.addNewTab0 = function(myTabTitle, myTabID, ownerID, type) {
	//   console.log("MContentPlugin.prototype.addNewTab ");
	//   console.log("map",map);
	...
	MContentPlugin.prototype.autoResize = function() {
		...
		if (!this.currTVWidth) this.currTVWidth = -1;
		if (!this.currBPVWidth) this.currBPVWidth = -1;
		if (!this.currAPVHeight) this.currAPVHeight = -1;
      ...
      tv.style.height = (h - 176) + "px";
		...
		var bp = document.getElementById("basicpropertysheet_" + this.options.pid);
		if(bp!=null) bp.style.height = (h - 176) + "px";
		var adpFrm = this.apSheet;
		var adpp = document.getElementById("adpropertysheetPane_" + this.options.pid);
		if (this.currAPVHeight == -1) {
			...
			if (h <= 427) {
				if (h > 186) {
					...
					if(adpp) adpp.style.height = (h - 186)+ "px";
					// 2 is up and bottom's border height;
          if(adpFrm) adpFrm.style.height = (h - 186 + this.apvhdDefaultHeight + 2) + "px";
				} else {
          if(adpp) adpp.style.height = 1 + "px";
          if(adpFrm) adpFrm.style.height = 41 + "px";
				}
			} else {
        if(adpp) adpp.style.height = this.apvDefaultHeight+ "px";
        if(adpFrm) adpFrm.style.height = (this.apvDefaultHeight + this.apvhdDefaultHeight+ 2) + "px";
			}
		} else {
        ...
        if(adpp) adpp.style.height = this.currAPVHeight+ "px";
        if(adpFrm) adpFrm.style.height = (this.currAPVHeight + this.apvhdDefaultHeight+ 2) + "px";
			}
		}

		// fixed column resize bar height
		if(this.lBar) this.lBar.style.height = (h - 130) + "px";
		if(this.rBar) this.rBar.style.height = (h - 130) + "px";
		// tabs height
			...
			if (document.body.style.cursor == "e-resize") {
				...
				if ((parseInt(this.rvPane.style.width) - dx) > this.minsize
					...
					this.bBar.style.width = (parseInt(this.bBar.style.width) + dx)
							+ "px";
					$("#tabcontents"+this.options.pid).children('DIV').each(
									function(entry) {
			...
			if (document.body.style.cursor == "n-resize") {
				...
				if ((parseInt(adFrm.style.height) - dy) > this.minsize
					...
					adFrm.style.height = (parseInt(adFrm.style.height) - dy)
							+ "px";
					var adpp = document.getElementById("adpropertysheetPane_"+this.options.pid);
					if(adpp) adpp.style.height = (parseInt(adpp.style.height) - dy)
							+ "px";
					$("#advancedproptabs")
							.children('DIV')
							.each(
					...
					var tbFrm = document.getElementById("tabcontents"+this.options.pid);
					console.log("tbFrm.style.height",tbFrm.style.height);
					tbFrm.style.height = (parseInt(tbFrm.style.height) + dy)
							+ "px";

					this.tabsHeight = this.tabsHeight + dy; // tabs height
					$("#tabcontents"+this.options.pid)
							.children('DIV')
							.each(
									function(entry) {
										// this.getAttribute("data") is id;
		...
		if (!this.mousepressed) {
			if (type!=="0") {
				if (document.body.style.cursor == "e-resize") {
					...
					// saving
					var apFrm = $("#adpropertysheetPane_"+this.options.pid).height();
