...
// 		this.init(options);
// 		console.log("searchAppPane-options",options);
// 	};
//
...
// 				+ options.pid, "终止执行", c1, "i", "fa fa-stop fa-lg");
...
// 					this.statusSelect.value, this.fromDateInput.value,
// 					this.toDateInput.value);
//       this.terminatebutton.setAttribute("disabled","");
...
//       this.terminatebutton.removeAttribute("disabled");
...
// 			$("#progressbar").show();
// 			var that = this;
// 			console.log("this.options.proinfo.uname",this.options.proinfo.uname);
// 			// service11/api5 发起流程 >>生成一个新的流程实例
// 			// ajaxPost(service.api1(5), {
//
// 		console.log("that.options",that.options);
// 		let param = {
// 			templateId : that.options.pid,// process ID
// 			userId : that.options.proinfo.uid,
// 			userDisplayName : that.options.proinfo.uname,
// 			bindEnClassName : that.options.proinfo.enClassName,
// 		};
//         ajaxPost(service.runAPI("/released-template/launch-byNewObj"),param).complete(function(data) {
//         console.log("data",data);
//         if(data.responseJSON.success){
//           alert("发起成功");
//           that.loading(that.options.pid, 1, 30, that.searchInput.value,
//             that.statusSelect.value, that.fromDateInput.value,
//             that.toDateInput.value);
//           that.terminatebutton.setAttribute("disabled","");
//         }else{
//           alert("发起失败");
//           // alert(data.responseJSON.status.split(":")[1]);
//         }
//         $("#progressbar").hide();
//
// 			});
...
// 	};
//
//   SearchConditionPanel.prototype.clearProcessSheet = function(table) {
...
//   };
//
//   SearchConditionPanel.prototype.selectRow = function(row) {
//     row.style.background = "#d1d1e0";
//   };
//   SearchConditionPanel.prototype.rowSelected = function(row) {
//     return row.style.background == "rgb(209, 209, 224)";
//   };
//
//   SearchConditionPanel.prototype.stopRow = function(evt){
//     var that = this;
//     if (this.instancelist.rows.length > 1) {
//       for (var i = 0; i < this.instancelist.rows.length; i++) {
//         if (this.instancelist.rows[i].id != "-1" && this.rowSelected(this.instancelist.rows[i])){
//           var tbode = this.instancelist.childNodes[1];
//           // 中止流程
//           var r = confirm("是否终止该流程实例？")
//           if (r == true) {
//             var that = this;
//             var pid = this.instancelist.rows[i].getAttribute("key");
//             console.log("流程实例id pid:",this.instancelist.rows[i].getAttribute("key"));
//             $("#progressbar").show();
//             // service11/api10, cmd="1", pid=流程实例id
//             // ajaxPost(service.api1(10), {
//             //   cmd: "1",
//             //   pid: pid,
//             ajaxPost(service.runAPI("/process-instance/"+pid+"/delete"),{
//             }).complete(function(data) {
//               if(data.success){
//                 alert("流程已终止");
//                 tbode.removeChild(tbode.childNodes[i-1]);
//               }else{
//                 alert("终止失败");
//               }
//               $("#progressbar").hide();
...
// 		$("#progressbar").show();
// 		var that = this;
// 		// ajaxGet(service.api(31), {
//     ajaxPost(service.adminAPI("/search-process-instance"), {
...
// 				this.createCell(3, processstatus[objs[i].status], row);//当前状态
// 				this.createCell(4, objs[i].launcher, row);//发起人
// 				// this.createCell(5, objs[i].idType, row);//
// 				// this.createCell(6, objs[i].idNumber, row);
// 				this.createCell(5, Utils.parseDateTime(objs[i].startTime), row);// 启动时间
// 				// this.createCell(8, objs[i].suspensionTime == -1 ? "" : Utils
// 				// 		.parseDateTime(objs[i].suspensionTime), row);
// 				// this.createCell(6, objs[i].updateTime == -1 ? "" : Utils
// 				// 		.parseDateTime(objs[i].updateTime), row); //修改时间
// 				// this.createCell(10, objs[i].server, row);
// 			}
// 			if (objs.length < 10) {
// 				for (var i = objs.length; i < 10; i++) {
