...
(function($, window, document, undefined) {
  ...
  var Editor = function(element, options) {
    ...
    this.stack = new CommandStack(this.options.proinfo.pid);
  ...
  Editor.prototype.loading = function(options) {
    var that = this;
    that.wfprocess = new WfProcessInstance();
    this.selected = [];
    // $("#progressbar").show();
    // api2data  service13/api2 通过id获得流程实例
    // getJSON(service.api(2), {
    getJSON(service.adminAPI("/wfprocess-instance/"+ options.id), {}).complete(function(data) {
      // console.log("api2data",data);
      that.wfprocess.parseFromJSON(JSON.parse(data.responseJSON.data));
      that.wfprocess.setContext(that.context);
      that.repaint();
      that.setPropertySheet();
      that.options.proinfo.setPropertySheet.$Message.info("流程加载完成");
  ...
  Editor.prototype.createToolbar = function(options) {
    ...
    this.refreshbutton = this.createTool(g1, "refresh" + this.options.id, "刷新",
  ...
  Editor.prototype.setPropertySheet = function() {
    ...
    if ( this.options.proinfo.setPropertySheet != null) {
      ...
      if(obj instanceof  WfProcess || obj instanceof WfProcessInstance){
