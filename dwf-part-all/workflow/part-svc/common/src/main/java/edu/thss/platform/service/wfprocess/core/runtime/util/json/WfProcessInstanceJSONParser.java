 ...
 * 
 */
package edu.thss.platform.service.wfprocess.core.runtime.util.json;

import edu.thss.platform.service.wfprocess.core.Location;
import edu.thss.platform.service.wfprocess.core.buildtime.util.json.WfProcessJSONParser;
import edu.thss.platform.service.wfprocess.core.buildtime.wfprocess.task.*;
...
import edu.thss.platform.service.wfprocess.core.communication.SecondaryComment;
...
public class WfProcessInstanceJSONParser extends WfProcessJSONParser {
	...
	public static WfProcessInstance parseWfProcessInstance(String jsonWfProcess) throws JSONException, Exception {
		...
		if (!obj.isNull("enClassInstanceId")) {
			newprocess.setEnClassInstanceId(obj.getString("enClassInstanceId"));
		}
        if (!obj.isNull("customData")) {
            newprocess.setCustomData(obj.getString("customData"));
        }
//		if (!obj.isNull("invitations")) {
//			newprocess.setInvitations(obj.getString("invitations"));
//		}
		if (!obj.isNull("invitations")) {
			...
			if (j3 != null && j3.length() > 0) {
				...
				newprocess.setInvitations(list.toArray(new String[list.size()]));
			}
		}
		if (!obj.isNull("copiers")) {
			JSONArray j3 = obj.getJSONArray("copiers");
			if (j3 != null && j3.length() > 0) {
				List<String> list = new ArrayList<String>();
				for (int i = 0; i < j3.length(); i++) {
					list.add(j3.getString(i));
				}
				newprocess.setCopiers(list.toArray(new String[list.size()]));
			}
		}
	...
	private static void parseChildren(JSONObject obj, WfProcessInstance newprocess) throws Exception {
		...
		if (obj.getString("classtypename").equals(ArrayDataVariable.class.getSimpleName())) {
			...
			if(!obj.isNull("launchFormName")){
				task.setLaunchFormName(obj.getString("launchFormName"));
			}
			if(!obj.isNull("startOperation")){
				task.setStartOperation(obj.getString("startOperation"));
			}
			if(!obj.isNull("launchFormContent")){
				task.setLaunchFormContent(obj.getString("launchFormContent"));
			}
			...
			if(!obj.isNull("endFormName")){
				task.setEndFormName(obj.getString("endFormName"));
			}
			if (!obj.isNull("endFormContent")) {
				task.setEndFormContent(obj.getString("endFormContent"));
			}
			if(!obj.isNull("endOperation")){
				task.setEndOperation(obj.getString("endOperation"));
			}
		//  EndPoint
//			if (!obj.isNull("participants")) {
//				task.setParticipants(obj.getJSONArray("participants"));
//			}
			newprocess.addChild(task);
			JSONArray jsonarr = obj.getJSONArray("accessibleVars");
			if (jsonarr.length() > 0) {
			...
			if(!obj.isNull("formName")){
				task.setFormName(obj.getString("formName"));
			}
			if (!obj.isNull("formContent")) {
				 task.setFormContent(obj.getString("formContent"));
			}
			if (!obj.isNull("participants")) {
				task.setParticipants(com.alibaba.fastjson.JSONArray.parseArray(obj.getJSONArray("participants").toString()));
			}
...
//			}
			...
			if(!obj.isNull("beforeOperation")){
				task.setBeforeOperation(obj.getString("beforeOperation"));
			}
			if(!obj.isNull("afterOperation")){
				task.setAfterOperation(obj.getString("afterOperation"));
			}

			if(!obj.isNull("goAhead")){
				task.setGoAhead(obj.getBoolean("goAhead"));
			}
			if(!obj.isNull("lastTaskId")){
				task.setLastTaskId(obj.getString("lastTaskId"));
			}
			newprocess.addChild(task);
	...
	private static void parseTransitionInstance(JSONArray jsonarr, WfProcessInstance newprocess) throws Exception {
	...
	public static Comment parseComment(JSONObject obj) throws Exception {
		...
		if (!obj.isNull("contentType"))
			cmt.setContentType(obj.getString("contentType"));
		if (!obj.isNull("messageType"))
