  ...
  <section v-if="t_preview" :addinName="name" ref="main" :style="{'width': colWidth}">
    <template v-if="args.structural_layout === 'vertical' && args.vertical">
          ...
          'padding-left':'16px','height': args.optlineHeight+'px', 'min-height': args.optlineHeight+'px', 'line-height': args.optlineHeight+'px','text-align': labelXAlign, 'vertical-align': labelYAlign, 'float':(args.useDictionary?args.labelFloat:'none')}">
              <span :style="{'background-color': args.label_color, 'width': '100%', 'display': 'inline-block','align-items': 'center', 'display': 'flex', 'justify-content': checkboxLabelXAlign}">
        ...
        <span :style="{'min-height': arg_height, 'width': '100%', 'display': 'inline-block','vertical-align': 'middle',
            ...
            'min-height': '100%'}">
          <van-checkbox-group :id="args.checkWrap" v-model="defaultValue"
                         @change="defaultValueChange"
                         direction='vertical'
                         :style="{'margin-right': '0px', 'min-height': arg_height, 'width': '100%', 'align-items': 'center', 'justify-content': checkboxMainXAlign, 'flex-wrap': 'wrap'}">
<!-- 'display': 'flex',  -->
            <van-cell-group style="width:100%;background-color:inherit;">
                    ...
                    <template v-if="!args.justifyAlign" slot="title" style="font-size: inherit;display: inline-block;">
                        <div style="display: flex;font-size: inherit;  display: table;table-layout: fixed;height:100%;width: 100%;">
                          <van-checkbox shape="square" :name="item.value" ref="" :icon-size="arg_icon_size" 
                          :style="{'width': arg_icon_size+' !important',float: 'left',height:'100%'}"   />

                          <span style="padding-left:5px;font-size: inherit;height:100%;
                            word-break:break-all;word-wrap:break-word;float:right;width:calc(100% - 25px);">
                            {{item.label}}</span>
                        </div>
                    </template>
                    <template v-if="args.justifyAlign" slot="title" style="font-size: inherit;">
                        <div style="display: flex;font-size: inherit;word-break:break-all;word-wrap:break-word;">
                            {{item.label}}
                        </div>
                    </template>
    ...
    <template v-else-if="args.structural_layout === 'vertical'">
      ...
      <!--
        ...
        -->
      <span :style="{'min-height': arg_height, 'width': '100%', 'display': 'inline-block','vertical-align': 'middle',
          'padding-left':'16px', 'padding-bottom':'8px','text-align': mainXAlign, 'color': args.main_fontColor}">
          <span :style="{'background-color': args.main_color, 'width': '100%',
          ...
          <van-checkbox-group :id="args.checkWrap" v-model="defaultValue"
                     ...
                     'min-height': args.optlineHeight+'px', 'width': '100%', 'align-items': 'center', 'display': args.vertical ? 'inline-grid' : 'flex', 'justify-content': checkboxMainXAlign, 'flex-wrap': 'wrap'}">
            <van-checkbox
                    shape="square"
                    :icon-size="arg_icon_size"
                    :style="{'min-height': args.optlineHeight+'px',}"
                    v-for="(item,index) in args.list"
                    :key="index"
                    :name="item.value">
                    <span class="checkItem" :style="{'max-width': mainWidth, 
                    'padding-bottom': '8px',
                    'font-size': args_fsize, 
                    'color': args.txt_fontColor, 
                    'margin-right':'8px'}">{{ item.label }}</span>
            </van-checkbox>
          </van-checkbox-group>
          </span>
      </span>
    ...
    <template v-else-if="args.structural_layout === 'horizontal'">
      ...
      <span :style="{'min-height': arg_height, 'width': mainWidth, 'display': 'inline-block','vertical-align': 'middle',
          'padding-left':'16px','text-align': mainXAlign, 'color': args.main_fontColor}">
          <span :style="{'background-color': args.main_color, 'width': '100%',
          'min-height': '100%', 'display': 'flex', 'vertical-align': mainYAlign}">
          ...
          <van-checkbox-group :id="args.checkWrap" v-model="defaultValue"
                         ...
                         :style="{'margin-right': '0px', 'min-height': args.vertical ?args.optlineHeight+'px':arg_height, 'width': '100%', 'align-items': 'center', 'display': args.vertical ? 'inline-grid' : 'flex',  'justify-content': checkboxMainXAlign, 'flex-wrap': 'wrap'}">
            <van-checkbox
                    shape="square"
                    :icon-size="arg_icon_size"
                    :style="{'min-height': args.optlineHeight+'px',}"
      ...
      <EditBox v-if="actEdit" :widgetAnnotation="widgetAnnotation" :editExtendInfo="editExtendInfo" ref="editbox" v-model="args"
        ...
        :targetclass="itemValue.data.targetClass">
          <div slot="attribute">
              ...
              <Input class="margin1" type="textarea" 
              @on-blur="createOptions"
              v-model="args.selfOptions" :disabled="args.useDictionary" placeholder="输入选项组内容, 如'a:1,b:2'" />
          ...
          <div slot="layout">
            ...
            <div class="margin1" style="margin: 10px 0 10px 0" v-show="args.vertical && args.structural_layout === 'vertical'">
                两端对齐
...
export default {
  ...
  watch: {
    ...
    'args.defaultValue'(val){
      ...
      else this.defaultValue = val && typeof val === 'string' ? val.split(',') : val;
    },
  ...
  methods: {
    ...
    switchDictionary(value) {
      ...
      //   lwrap.style.optlineHeight = 'normal';
      // }
    ...
    setHeight() {
        ...
        //     lwrap.style.optlineHeight = actHeight.height;
        //   }
