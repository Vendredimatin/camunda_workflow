<template>
  ...
  >
    <MobileFormAddinList
      ...
      v-if="addin.self.dropTarget === col.id"
      :basicArgs="basicArgs"
      v-bind="addinProps"
      :key="addin.self.uuid"
      :addin="addin"
        ...
        switch (type) {
          case 'added':
            ...
            if (evt.added.element.addin) {
              //非收藏控件拖入
              evt.added.element.addin.self.dropTarget = this.col.id;
              this.addin.splice(evt.added.newIndex, 0, evt.added.element.addin);
              this.originAddin.elements = this.originAddin.elements.filter(element => element.self.dropTarget !== this.col.id).concat(this.addin);
