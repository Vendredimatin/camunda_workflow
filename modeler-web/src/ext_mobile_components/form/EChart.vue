<template>
    ...
    <section v-if="t_preview" :addinName="name" ref="main" :style="{'width': arg_width}">
        ...
        <Modal v-model="editing" title="编辑" width="80%" :mask-closable="false" @on-ok="run" ok-text="运行">
            <editor v-model="args.code" @init="editorInit" lang="javascript" theme="chrome" width="100%" height="600"></editor>
        </Modal>
        <div ref="echartE">
...
export default {
  ...
  created() {
    ...
    methods: {
        ...
        run() {
            let that = this;
            let params = { echarts: this.echarts, myChart: this.chart, store: this.store, $: $,
                dwf_axios: dwf_app_axios, dwf_modeler_axios: axios, axios: Axios.create({ baseURL: "", withCredentials: true }),
                GetAddinById: (id) => { return that.GetAddinById(this.itemValue.data, id); },
        ...
        complete() {
            ...
            let that = this;
            setTimeout(() => {that.chartResize()}, 100);
        },
