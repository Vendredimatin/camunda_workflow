<template>
  ...
  <div>
    ...
    >
        ...
        <Col span="8">
          ...
          <Form :model="createForm" :rules="rowValidate" ref="createForm">
            <FormItem prop="viewName" label="表单名(英文名)">
              <br>
              <Input :disabled="viewDisabled" placeholder="表单名" style="width: 300px" v-model="createForm.viewName" id="entityFormCreateViewName"/>
            </FormItem>

            <FormItem prop="viewDisplayName" label="显示名(中文名)">
              <br>
              <Input placeholder="显示名" style="width: 300px" v-model="createForm.viewDisplayName" id="entityFormCreateViewDisplayName"/>
            </FormItem>

            <FormItem prop="note" label="备注">
              <br>
              <Input v-model="createForm.note" type="textarea" :autosize="true" placeholder="Enter something..."
                     id="entityFormCreateNote"
      ...
      <div slot="footer">
        ...
        <Button type="primary" size="large"
                ...
                @click="goToFormCreate('create',createForm.chosenClass,chosenViewId,createForm.viewName,createForm.viewDisplayName,createForm.note, createForm.chosenType, 'entity')">
          确认
        </Button>
      </div>
    </Modal>
    <div style="width: calc(100% - 20px);margin: 20px auto">
      <!--搜索框 start-->
      <Input v-model="searchKey" icon="md-search" placeholder="输入关键词查询" style="width: 200px;float: right;" id="entityFormSearchInput"></Input>
...
<script>
  ...
  import {deleteView, getAllEntities, getRelationNameF, getViews, getView } from '../../api/others'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rowValidate: {
          ...
          chosenClass: [
            {required: true, message: "必须选择一个实体类", trigger: "blur"}
          ],
          note: [
            {
              pattern: /^[\S\s]{1,500}$/,
      ...
      searchData() {
        ...
        if (sk) {
          data.forEach(function (obj) {
            if (String(obj['className']).toLowerCase().indexOf(sk) > -1 || String(obj['displayName']).toLowerCase().indexOf(sk) > -1 || String(obj['zoneName']).toLowerCase().indexOf(sk) > -1) {
              newData.push(obj);
            }
          });
          data = newData;
    ...
    methods: {
      ...
      actDev(ind) {
        // if(this.viewDisabled) return false;
        this.actDevice = ind;
      ...
      goToNewFormCreate(flag, className, viewName, displayName, note, type) {
        this.root.goToNewFormCreate(flag, className, viewName, '', '', displayName, note, type, 'entity');
      },
      ...
      goToFormCreate(operation, className, formOid, formName, formDisplayName, formNote, formType) {
        this.$refs['createForm'].validate(async valid => {
          if (valid) {
            ...
            await this.handleTemplate(targetClass, viewName, formDisplayName || '', false, '', formNote || '', formType || 'PC')
            let url = `${Config.protocol}//${Config.host}/${Config.baseRouteName}/forms/${targetClass}/${viewName}?token=${this.$store.state.user.token}&formType=${formType || 'PC'}&classType=entity&displayName=${formDisplayName || ''}&note=${formNote || ''}&createForm=true`;
            a.href = url;
            a.target="_blank";
            a.click();
      ...
      ,
      initData() {
        const noNeedClass = ['Component', 'ComponentGroup']
        this.tableSpin = true;
        ...
        getAllEntities({"needOperationCount": false}).then((res) => {
          ...
          _self.formData = entities.filter(entity => !noNeedClass.includes(entity.className));
