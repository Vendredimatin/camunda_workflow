<template>
    <div>
    <Modal v-model="chooseClassAndAttributes" title="导出数据" id="exportDataModal">
        <h3>选择目标类</h3>
        <Select @on-change="getClassNameValue" v-model="AllChosedTargetClassName"  filterable id="exportDataChooseClassName">
            <Option :value="item.className" v-for="item in AllTargetClassesName" v-bind:key="item.id">{{item.className+" : "+item.displayName}}</Option>
        </Select>
        <h3>系统属性</h3>
        <div v-if="AllTargetClassSystemAttributes.length > 0" style="border: 1px solid #88AAE1;" id="exportDataSysAttr">
        ...
        <h3>类属性</h3>
        <div v-if="AllTargetClassCustomAttributes.length > 0" style="border: 1px solid #88AAE1;" id="exportDataDefAttr">
    ...
    <Button type="primary" :disabled="!hasClass" style="display:inline-block; margin: 5px;" @click="onClickExportDataButton" id="exportDataButton">导出数据</Button>
    </div>
</template>
...
const name = "InternalEntityDataExport";
...
export default {
    ...
    methods: {
        exportData(){
            ...
            dataExportFunc(this.AllChosedTargetClassName, AllTargetAttributes, false)
