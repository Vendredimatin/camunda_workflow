    ...
    <div style="float: left; margin: 10px 10px 0">
      ...
      >
        <Form ref="editRowForm" :model="editRowData" :rules="rowValidate" :label-width="120">
          <FormItem label="数据连接名称" prop="dataSourceName">
            <Input v-model="editRowData.dataSourceName" placeholder="输入数据连接名" id="dataLinkModalDataSourceName"></Input>
          </FormItem>
          <FormItem label="数据连接类型" prop="dataSourceType">
            <Select v-model="editRowData.dataSourceType" placeholder="选择数据连接类型" filterable id="dataLinkModalDataSourceType">
...
<!--          <FormItem label="数据连接文件类型" prop="dataFileFormat">-->
<!--            <Select v-model="editRowData.dataFileFormat" placeholder="选择数据连接文件类型" filterable>-->
<!--              <Option v-for="item in allDataFileFormats" :value="item" :key="item" :label="item">-->
<!--                <span>{{ item }}</span>-->
<!--              </Option>-->
<!--            </Select>-->
<!--          </FormItem>-->
...
<!--          <FormItem label="管理界面地址" prop="workBenchUrl">-->
<!--            <Input v-model="editRowData.workBenchUrl" placeholder="输入管理界面地址"></Input>-->
<!--          </FormItem>-->
<!--          <FormItem label="参数One" prop="paramOne">-->
<!--            <Input v-model="editRowData.paramOne" placeholder="输入参数One"></Input>-->
<!--          </FormItem>-->
<!--          <FormItem label="扫描层数" prop="folderDepth">-->
<!--            <Input v-model="editRowData.folderDepth" placeholder="输入扫描层数"></Input>-->
<!--          </FormItem>-->
        </Form>
        <div slot="footer">
...
export default {
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            props: { placement: 'bottom', maxWidth: 362, transfer: true }
                ...
                params.row.description.length > 16 ? params.row.description.slice(0, 16) + "..." : params.row.description,
                h('span', { slot: 'content', style: { whiteSpace: 'normal', wordBreak: 'break-all' } },
  ...
  methods: {
    ...
    addRow() {
      this.$refs["editRowForm"].validate(valid => {
        if (valid) {
          ...
          DataSourceMgr.createDataSource(this.editRowData)
            .then(res => {
              this.getAllRows();
    ...
    deleteRowOnOk() {
      ...
      DataSourceMgr.deleteDataSource(this.currentData.oid, this.cascade)
        .then(res => {
          this.getAllRows();
          if (
      ...
      this.$refs["editRowForm"].validate(valid => {
        if (valid) {
          ...
          DataSourceMgr.getDataSource(this.editRowData.oid)
            .then(res => {
              DataSourceMgr.updateDataSource(this.editRowData)
                .then(res => {
                  this.getAllRows();
                  this.editRowModalLoading = false;
                  this.editRowModal = false;
