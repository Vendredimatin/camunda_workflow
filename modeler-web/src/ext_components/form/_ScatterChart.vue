<template>
    <section :addinName="name" v-if="t_preview" ref="main" :style="{'width': arg_width, 'float': 'left'}">
        ...
        <h2 v-show="hasSeries" style="font-size: 18px; text-align: center; color: #333;font-weight: bold;">散点图</h2>
                ...
                <div slot="attribute">
                    <div class="editsection">
                        <Form :model="args" :label-width="80">
                            ...
                            <Select v-if="reloadX" v-model="args.nameAttribute" filterable clearable>
                            ...
                            <p class="margin1">x轴单位</p>
                            <Input class="margin1" v-model="args.xAxisSelfName"></Input>

                            <p class="margin1">y轴</p>
                            <Select v-if="reloadX" v-model="args.valueAttribute" filterable clearable>
                              <OptionGroup v-for="group in groupAttrList" :label="group.groupName" :key="group.groupName">
                                <Option v-for="(item, index) in group.attrList" :key="index" :value="`${group.groupType}${item.attributeName}`" :label="item.displayName">
                                  <span>{{ item.displayName }}</span>
                                  <span style="float:right;color:#ccc">{{ item.attributeName }}</span>
                            ...
                            <p class="margin1">y轴单位</p>
                            <Input class="margin1" v-model="args.yAxisSelfName"></Input>
                            <br>
                            <br>
...
export default {
  ...
  async mounted() {
    ...
    if (this.t_preview) {
      ...
      //   this.args.bindTargetClass.split('&')[0],
      //   this.parsedStr
      // ); //获取数据，整理数据
    ...
    async arg_bindClass(val) {
      ...
      if(val != undefined && val != '' && val != 'undefined') {
        ...
        if(val.split('&')[1] == 'e') {
          ...
          this.groupAttrList.push(groupLItem, groupReItem, groupRItem);
