<template>
    <section :addinName="name" v-if="t_preview" ref="main" :style="{'width': arg_width, 'float': 'left'}">
        ...
        <h2 v-show="hasSeries" style="font-size: 18px; text-align: center; color: #333;font-weight: bold;">饼图</h2>
                ...
                <div slot="attribute">
                    <div class="editsection">
                        <Form :model="args" :label-width="80">
                            ...
                            <p class="margin1">数值形式</p>
                            <RadioGroup v-model="args.numType">
                                <Radio label="number">数值</Radio>
                                <Radio label="percent">百分比</Radio>
                            </RadioGroup>

                            <p class="margin1">图表形式</p>
                            <RadioGroup v-model="args.pieType">
                                <Radio label="pie">饼图</Radio>
                                <Radio label="circle">圆环图</Radio>
                            </RadioGroup>
                            <p class="margin1">图表内半径</p>
                            <InputNumber v-model="args.insideRadiu" :max="100" :min="1">
                            </InputNumber>
                            <p v-show="args.pieType == 'circle'" class="margin1">图表外半径</p>
                            <InputNumber v-show="args.pieType == 'circle'" v-model="args.outsideRadiu" :max="100" :min="1">
                            </InputNumber>
                            <br>
                            <br>
  ...
  methods: {
    ...
    render(){
      if(!this.args.nameAttribute) {
        ...
        this.hasSeries = false;
        let finalRadiu = '55%';
        if(this.args.pieType == 'pie') {
          finalRadiu = this.args.insideRadiu ? `${this.args.insideRadiu}%` : '55%';
        } else {
          finalRadiu = [this.args.insideRadiu ? `${this.args.insideRadiu}%` : '55%', this.args.outsideRadiu ? `${this.args.outsideRadiu}%` : '70%'];
        }
        if(this.args.numType == 'number') {
          this.args.option.tooltip.formatter = '{b}：{c}';
        } else {
          this.args.option.tooltip.formatter = '{b}：{d}%';
        }
        this.args.option.series = [];
