...
<span v-show="t_edit" ref="edit">
                ...
                <div slot="attribute">
                    <div class="editsection">
                        <Form :model="args" :label-width="80">
                            ...
                            <Select v-model="args.valueAttribute" multiple filterable>
                                <Option v-for="(item, index) in filter_attributes" :key="index" :value="item.attributeName" :label="item.displayName">
                                  <span>{{ item.displayName }}</span>
                                  <span style="float:right;color:#ccc">{{ item.attributeName }}</span>
                                </Option>
        ...
        <Modal
          ...
          @on-cancel="cancelScript">
            <MonacoEditor v-if="showScriptModal" v-model="args.echartScript" @init="editorInit" lang="javascript" theme="chrome" width="100%" height="600"></MonacoEditor>
        </Modal>
        <!-- 最大化编辑器ENDING -->
    </section>
    ...
    render(){
      if(!this.args.nameAttribute) {
        ...
        this.args.option.xAxis = {
            ...
            data: (() => {
                var res = [];
                this.args.option.dataset.source.slice(1).forEach(x => (res.push(x[this.nameIndex])));
                return res;
            })(),
            axisLabel: {
        ...
        this.args.option.yAxis = { type: "value" };
        this.args.option.series = [];

        console.log("this.valueAttribute", this.args.valueAttribute);
        ...
        for(let index of this.valueIndex){
            this.args.option.series.push({
                ...
                data: (() => {
                    var res = [];
                    this.args.option.dataset.source.slice(1).forEach(x => (res.push(x[index])));
                    return res;
                })(),
            });
        }
