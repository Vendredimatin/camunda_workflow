...
<span v-show="t_edit" ref="edit">
                ...
                <div slot="attribute">
                    <div class="editsection">
                        <Form :model="args" :label-width="80">
                            ...
                            <Select v-model="args.longitudeAttribute" filterable>
                            ...
                            <Select v-model="args.latitudeAttribute" filterable>
                                <Option v-for="(item, index) in filter_attributes" :key="index" :value="item.attributeName" :label="item.displayName">
                                  <span>{{ item.displayName }}</span>
                                  <span style="float:right;color:#ccc">{{ item.attributeName }}</span>
                                </Option>
                            ...
                            <Select v-model="args.nameAttribute" filterable>
                                <Option v-for="(item, index) in filter_attributes" :key="index" :value="item.attributeName" :label="item.displayName">
                                  <span>{{ item.displayName }}</span>
                                  <span style="float:right;color:#ccc">{{ item.attributeName }}</span>
                                </Option>
                            ...
                            <Select v-model="args.valueAttribute" filterable>
                                <Option v-for="(item, index) in filter_attributes" :key="index" :value="item.attributeName" :label="item.displayName">
                                  <span>{{ item.displayName }}</span>
                                  <span style="float:right;color:#ccc">{{ item.attributeName }}</span>
                                </Option>
    ...
    <section v-else :addinName="name">
        <span style="text-align:center">
            ...
            <Tooltip content="散点地图" style="width: 100%;" :transfer="true">散点...</Tooltip>
        </span>
    </section>
...
export default {
  ...
  methods: {
    ...
    render(){
        ...
        if(!this.args.longitudeAttribute) {
        this.$Notice.warning({
            title: '提示',
            desc: '请先选择经度'
        });
        }
        else if(!this.args.latitudeAttribute) {
        this.$Notice.warning({
            title: '提示',
            desc: '请先选择纬度'
        });
        }
      else if(!this.args.nameAttribute) {
        this.$Notice.warning({
          title: '提示',
          desc: '请先选择标签'
        });
        ...
        this.args.option.series = [{
              ...
              data: (() => {
                  let res = [];
                  this.args.option.dataset.source.slice(1).forEach(x => {
                      res.push({
                          name: x[this.nameIndex],
                          value: (() => {
                              let r = [];
                              r.push(x[this.longitudeIndex]);
                              r.push(x[this.latitudeIndex]);
                              r.push(this.valueIndex >= 0 ? x[this.valueIndex] : 50);
                              return r;
                          })(),
                      })
                  })
            ...
            },
            {
              ...
              data: (() => {
                  var res = [];
                    this.args.option.dataset.source.slice(1).forEach(x => (res.push({name:x[this.nameIndex], value:this.valueIndex >= 0 ? x[this.valueIndex] : 50})));
                    return res;
              })(),
            }];
