...
export default {
  ...
  created() {
    ...
    if (this.attributes){
      ...
      this.args.paralleAttr = this.attributes;
    }
  },
  // 生命周期函数，在dom元素生成之后调用
  async mounted() {
  ...
  methods: {
    ...
    render(){
      if(this.args.valueAttribute.length == 0){
        this.$Notice.warning({
          title: '提示',
          desc: '请先选择数据轴'
        });
        ...
        for (let i = 0; i < this.args.valueAttribute.length; i++) {
            let attr = this.attributes.filter(x => x.attributeName === this.args.valueAttribute[i])[0];
