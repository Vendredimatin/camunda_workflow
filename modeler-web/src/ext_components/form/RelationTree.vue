    ...
    <section v-if="t_preview" :addinName="name" :style="{'width': colWidth, 'height': arg_height}" ref="main">
        ...
        <div v-show="isHasTree" :style="{'position': 'relative', 'width': '100%', 'height': 'calc(100% - 30px)', 'overflow-y': 'auto',  'background': '#fff', 'padding': '10px'}">
            <div v-show="args.showRefresh || args.showSearchInput" style="height: 40px; border-bottom: 1px solid #ddd;">
                <Row>
                    <Col span="20">
                        <Input v-show="args.showSearchInput" icon="ios-search" />
                    </Col>
...
<span v-show="t_edit" ref="edit">
            ...
            :targetclass="itemValue.data.targetClass">
                <div slot="attribute">
                    ...
                    <Select class="margin1 editbox-targetClass" v-model="args.targetClass" filterable clearable @on-change="changeClass">
                    ...
                    <p class="margin1">关联类</p>
                    <Select v-if="reloadR" class="margin1 editbox-targetClass" v-model="args.targetRelationClass" filterable clearable @on-change="changeRelationClass">
                    ...
                    <Select class="margin1" v-model="args.iconName" filterable clearable>
                    ...
                    <Select class="margin1" v-model="args.iconCName" filterable clearable>

                        <OptionGroup label="关联实体类类属性">
                            <Option v-for="item in args.targetRelationEntitiesAttributes.selfAttr" :key="item.id" :value="item.attributeName">{{ item.displayName }} {{ item.attributeName }}</Option>
                        </OptionGroup>
                        <OptionGroup label="关联实体类系统属性">
                            <Option v-for="item in args.targetRelationEntitiesAttributes.sysAttr" :key="item.id" :value="item.attributeName">{{ item.displayName }} {{ item.attributeName }}</Option>
                        </OptionGroup>

                        <OptionGroup label="关联类类属性">
                            <Option v-for="item in args.targetRelationClassAttributes.selfAttr" :key="item.id" :value="`${item.attributeName}&r`">{{ item.displayName }} {{ item.attributeName }}</Option>
                        </OptionGroup>
                        <OptionGroup label="关联类系统属性">
                            <Option v-for="item in args.targetRelationClassAttributes.sysAttr" :key="item.id" :value="`${item.attributeName}&r`">{{ item.displayName }} {{ item.attributeName }}</Option>
                        </OptionGroup>
                ...
                <div slot="operation">
                    <div class="margin1" style="margin: 10px 0 10px 0; font-weight: bold;">
                        <span>根节点操作配置</span>
                        <Button size="small" type="text" icon="md-add" @click="createRootOpr" style="position: relative; top: -2px;"></Button>
                    </div>
                    ...
                    <div class="margin1">
                        <div v-for="(item, index) in args.rootOperation" :key="index" class="margin1">
                            ...
                            <div style="display: inline-block;width: calc(100% - 56px)">
                                <Input v-if="item.tmp" v-model="item.name" style="font-size: 12px;"/>
                                <span v-else style="font-size: 12px;">{{ item.name }}</span>
                            </div>
                            <div style="display:inline-block;width: 56px">
                                <div>
                                    <Button v-if="item.tmp" style="margin-left: 3px ;margin-right: 3px;" size="small" type="primary" icon="md-checkmark" @click="confirmEvent(index)"></Button>
                                    <Button v-else style="margin-left: 3px ;margin-right: 3px;" size="small" type="primary" icon="md-create" @click="editOpr(item, index, 'root')"></Button>
                                    <Button style="display: inline-block;" size="small" type="gohst" icon="md-close" @click="cancelEvent(index)"></Button>
                                </div>
                            </div>
                  ...
                  <!-- 子节点操作配置 -->
                    <div class="margin1" style="margin: 10px 0 10px 0; font-weight: bold;">
                        <span>子节点操作配置</span>
                        <Button size="small" type="text" icon="md-add" style="position: relative; top: -2px;" @click="createChildOpr"></Button>
                    </div>
                    ...
                    <div class="margin1">
                        <div v-for="(item, index) in args.childOperation" :key="index" class="margin1">
                            ...
                            <div style="display:inline-block;width: 56px">
                                <div>
                                    <Button v-if="item.tmp" style="margin-left: 3px ;margin-right: 3px;" size="small" type="primary" icon="md-checkmark" @click="confirmChildEvent(index)"></Button>
                                    <Button v-else style="margin-left: 3px ;margin-right: 3px;" size="small" type="primary" icon="md-create" @click="editOpr(item, index, 'child')"></Button>
                                    <Button style="display: inline-block;" size="small" type="gohst" icon="md-close" @click="cancelChildEvent(index)"></Button>
                                </div>
                            </div>
        ...
        <Modal
                ...
                <CheckboxGroup v-model="args.selected_relation_entities_attributes" @on-change="clickRelationEntitiesAttr">
                    <Collapse simple v-model="selfENGroup">
                        <Panel name="1">
                            ...
                            <div slot="content">
                                <Checkbox
                                    v-for="item in args.targetRelationEntitiesAttributes.sysAttr"
                                    :key="item.id"
                                    :label="`obj.${args.reDirection}_${item.attributeName}`"
                                    class="commonLabel"
                                    >{{ item.displayName }}
                                </Checkbox>
                            </div>
                        ...
                        <Panel name="2">
                            ...
                            <div slot="content">
                                <Checkbox
                                    v-for="item in args.targetRelationEntitiesAttributes.selfAttr"
                                    :key="item.id"
                                    :label="`obj.${args.reDirection}_${item.attributeName}`"
                                    class="commonLabel"
                                    >{{ item.displayName }}
            ...
            <p class="attrTitle" v-show="isChildLable">关联类属性:</p>
            <div class="attrBox" v-show="isChildLable">
                <CheckboxGroup v-model="args.selected_relations_attributes" @on-change="clickRelationAttr">
                    <Collapse simple v-model="selfREGroup">
                        <Panel name="1">
                            关联类系统属性
                            <div slot="content">
                                <Checkbox
                                    v-for="item in args.targetRelationClassAttributes.sysAttr"
                                    :key="item.id"
                                    :label="`obj.relation_${item.attributeName}`"
                                    class="commonLabel"
                                    >{{ item.displayName }}
                                </Checkbox>
                            </div>
                        ...
                        <Panel name="2">
                            ...
                            <div slot="content">
                                <Checkbox
                                    v-for="item in args.targetRelationClassAttributes.selfAttr"
                                    :key="item.id"
                                    :label="`obj.relation_${item.attributeName}`"
                                    class="commonLabel"
                                    >{{ item.displayName }}
    ...
    <section v-else :addinName="name">
        <span style="text-align:center">
            ...
            <Tooltip class="form-addin-name" content="关联结构树" style="width: 100%;" :transfer="true">关联树</Tooltip>
        </span>
    </section>
</template>
  ...
  created() {
    ...
    methods: {
        ...
        async initData() {
            ...
            if(this.args.targetRelationClass != undefined || this.args.targetRelationClass != '') {
                this.changeRelationClass(this.args.targetRelationClass)
            }
        ...
        confirmSetOpr() {
            ...
            if(this.oprForm.name == '') {
                ...
                if(this.args.targetRelationClass && this.args.rootLabel != '' && this.args.childrenLabel != '') {
                    this.updateTree();
                }
        ...
        */
        ...
        // 自定义 子 lable 2: 标签 4: 条件   (关联类)
        clickRelationAttr(value) {

            if(this.isRootLable == '2') {

                if(value.length > this.lastRelationAttr.length) {
                    this.selfRootLable = this.selfRootLable + value[value.length - 1] + '/';
                } else {
                    let delAttr = this.lastRelationAttr.concat(value).filter(v => this.lastRelationAttr.includes(v) && !value.includes(v));
                    this.selfRootLable = this.selfRootLable.replace(`${delAttr[0]}/`,'');
                }

                this.lastRelationAttr = value;

            } else {

                if(value.length > this.lastQueryRelationAttr.length) {
                    this.selfRootLable = this.selfRootLable + value[value.length - 1] + ' ';
                } else {
                    let delAttr = this.lastQueryRelationAttr.concat(value).filter(v => this.lastQueryRelationAttr.includes(v) && !value.includes(v));
                    this.selfRootLable = this.selfRootLable.replace(`${delAttr[0]} `,'');
                }

                this.lastQueryRelationAttr = value;
            }
        ...
        /**
         * @description 更改实体类 联动关联类变化
        */
        async changeClass(value) {

            if(this.itemValue.data.isRelation) {
                value = this.args.targetClass;
            }
            ...
            var r1 = this.LeftRelations(value);
            var r2 = this.RightRelations(value);

            this.l_relation = r1.length > 0 ? r1.map(x => { return x.basic }) : [];
            this.r_relation = r2.length > 0 ? r2.map(x => { return x.basic }) : [];

            if(this.args.targetRelationClass != undefined && this.args.targetRelationClass != '') {

                const curRE = this.args.targetRelationClass.split('&')[1];
                const curREname = this.args.targetRelationClass.split('&')[0];
                if(curRE == 'l') {

                    let Lobj = this.LeftEntities(this.args.targetClass);
                    this.args.reEnName = this.l_relation.find(x => { return x.className == curREname}).leftClass;
                    await this.queryEntity(this.args.reEnName);
                    let allReEnAttr = Lobj.find(x => { return x.basic.className == this.args.reEnName}).attributes;
                    this.args.targetRelationEntitiesAttributes.sysAttr = allReEnAttr.filter(item => SYS_ENTITY_ATTRIBUTES(item) !== -1);
                    this.args.targetRelationEntitiesAttributes.selfAttr = allReEnAttr.filter(item => SYS_ENTITY_ATTRIBUTES(item) === -1);

                } else {

                    let Robj = this.RightEntities(this.args.targetClass);
                    this.args.reEnName = this.r_relation.find(x => { return x.className == curREname}).rightClass;
                    await this.queryEntity(this.args.reEnName);

                    let allReEnAttr = Robj.find(x => { return x.basic.className == this.args.reEnName}).attributes;
                    this.args.targetRelationEntitiesAttributes.sysAttr = allReEnAttr.filter(item => SYS_ENTITY_ATTRIBUTES(item) !== -1);
                    this.args.targetRelationEntitiesAttributes.selfAttr = allReEnAttr.filter(item => SYS_ENTITY_ATTRIBUTES(item) === -1);
        ...
        /**
         * @description 更改关联类 联动关联类属性变化
        */
        async changeRelationClass(value) {

            if(value) {

                if(value.split('&')[1] == 'l') {
                    this.args.reDirection = 'left';
                } else {
                    this.args.reDirection = 'right';
                }
                await this.queryRelation(value.split('&')[0]);
       ...
       async setLable(status) {
           ...
           if(status == '1') {
               ...
               this.editLabelSpan = '编辑节点标签:'
               if(this.args.targetRelationClass == '') {

                   this.$Message.warning('请先选择所需关联类');
                   ...
                   if(this.args.childrenLabel != '') {
                       ...
                       let allCheckObj = this.args.childrenLabel.split('/');
                        ...
                        this.args.selected_entities_attributes = allCheckObj;
                        this.args.selected_relations_attributes = allCheckObj;

                        if(this.lastRelationEntitiesAttr.length == 0) {
                            this.lastRelationEntitiesAttr = allCheckObj;
                        }
                        if(this.lastRelationAttr.length == 0) {
                            this.lastRelationAttr = allCheckObj;
                        }
                   ...
                   if(curRE == 'l') {
                    ...
                    // 获取当前所选目标关联类的属性
                    let allReAttr = this.RelationAttrList(this.args.targetRelationClass.split('&')[0]);
                    this.args.targetRelationClassAttributes.sysAttr = allReAttr.filter(item => SYS_RELATION_ATTRIBUTES(item) !== -1);
                    this.args.targetRelationClassAttributes.selfAttr = allReAttr.filter(item => SYS_RELATION_ATTRIBUTES(item) === -1);
            ...
            if(this.args.targetRelationClass == '') {
                ...
                if(this.args.reDirection == 'right') {
                    filterObj.hideRelationClass = ['left']
                } else {
                    filterObj.hideRelationClass = ['right']
                }
                this.$refs.filterQuery_modal.initModal(this.args.rootQuery, this.args.targetClass.split('&')[0], filterObj, false, 'en');
            ...
            if(this.args.targetRelationClass == '') {
                ...
                if(this.args.reDirection == 'right') {
                    filterObj.hideRelationClass = ['relation','right']
                } else {
                    filterObj.hideRelationClass = ['relation','left']
                }
                this.$refs.filterQuery_modal.initModal(this.args.childrenQuery, this.args.targetRelationClass.split('&')[0], filterObj, false, 're');
       ...
       */
        ...
        */
        async updateTree() {
            ...
            if(this.args.targetRelationClass == '') {
                ...
                if(this.args.rootQuery != '') {
                    ...
                    parseRootQuery = this.commonTransLabel(this.args.rootQuery);
                    if(this.args.str_re == 'l') {
                ...
                // 转换关联实体类为最终query格式 $obj_r_obj.age => and 关联实体类英文名.plt_age
                // const re_en_name = 'and ' + this.args.reEnName + '.plt_';

                // 转换关联类为最终query格式 $obj_r.age => and 关联实体类英文名.plt_age
                // const re_name = 'and ' + relationName + '.plt_';
                ...
                if(this.args.childrenQuery != '') {
                    ...
                    let parseChildQuery =  this.commonTransLabel(this.args.childrenQuery);
                    if(this.args.str_re == 'l') {
                ...
                if(this.args.str_re == 'l') {
                    ...
                    let temCRabel = this.args.childrenLabel.replace(/obj\.right\_/g, 'right_');
                    transChildrenLabel = temCRabel.replace(/obj\.relation\_/g, 'relation_');
        ...
        async getNodeChild(item) {
            ...
            let nodeResult = await getEJobj(this.args.relationName, nodeQuery, true);
        ...
        async loadData(item, callback) {
            ...
            if(this.args.str_re == 'l') {
                ...
                nodeQuery.condition = this.args.childrenQuery == '' ? `and rightclass.plt_oid = '${item.left_oid}'` : `and rightclass.plt_oid = '${item.left_oid}' ${this.args.transChildrenQuery}`;
                // queryChildrenData.query.query = `and rightclass.plt_oid = '${item.left_oid}' ${this.args.transChildrenQuery}`;
                // queryChildrenData.rightClass = item.relation_rightClass;
                // queryChildrenData.leftClass = item.relation_leftClass;
                ...
                nodeQuery.condition = this.args.childrenQuery == '' ? `and leftclass.plt_oid = '${item.right_oid}'` : `and leftclass.plt_oid = '${item.right_oid}' ${this.args.transChildrenQuery}`;
                // queryChildrenData.query.query = `and leftclass.plt_oid = '${item.right_oid}' ${this.args.transChildrenQuery}`;
                // queryChildrenData.leftClass = item.relation_rightClass;
                // queryChildrenData.rightClass = item.relation_leftClass;
