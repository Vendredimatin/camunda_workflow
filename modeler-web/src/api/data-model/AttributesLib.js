...
export default {
  ...
  getDicts: () => {
    return axios.get(`dicts`).then(res => {
      return res.data;
    });
  }
};
...
export const getAttributeBindClasses = async function(name) {
  return await axios.get(`meta/attributes/${name}/bind-classes`).then(res => {
    return res.data;
  });
};
