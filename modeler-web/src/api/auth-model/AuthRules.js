...
export default {
  ...
  getAllRulesByCondition: condition => {
    return axios.get(`auth/rules?condition=${condition}`)
  },
  getConflictRules: () => {
  ...
  deleteRuleByClassOrAuthItem: (className, authItem) => {
    return axios.post(`auth/rules-delete?className=${className}&authItem=${authItem}`);
  },
  deleteRules: data => {
    return axios.post(`auth/rules-delete-batch`, data).then(res => {
      return res.data;
    });
  },
  getInvalidRules: () => {
