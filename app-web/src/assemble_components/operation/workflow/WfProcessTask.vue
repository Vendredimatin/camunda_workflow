    ...
    <div style="margin: 10px">
      <div style="height: 50px; margin-bottom: 2px">
        ...
        <Button
          type="primary"
          class="self-btn"
          :disabled="taskStatus != 2 && taskStatus != 4"
        ...
        <Input
          ...
          v-model="searchInput"
          style="width: auto; margin: 5px; float: right"
        />

        <Button
          class="self-btn"
          type="error"
          :disabled="taskStatus != 2 && taskStatus != 4 && taskStatus != 3"
      ...
      <Modal v-model="assignModal" :mask-closable="false" title="选择用户">
        ...
        />
        <div slot="footer">
          <Button type="text" size="large" @click="closeAssignMoal"
            >取消</Button
          >
          <Button type="primary" size="large" @click="onConfirmAssign"
            >确认</Button
          >
        </div>
      </Modal>
      ...
      <div style="margin: 10px">
        ...
        <div class="self-tag" style="float: right; line-height: 34px">
          总共{{ totalNumber }}条
        </div>
      </div>
    </div>
...
export default {
    ...
    return {
      ...
      taskColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            var s;
            if (params.row.taskInstanceStatus == 1) s = "待领取";
            else if (params.row.taskInstanceStatus == 2) s = "待提交";
            else if (params.row.taskInstanceStatus == 3) s = "我参与的";
            else if (params.row.taskInstanceStatus == 4) s = "移交给我的";
            return h("div", s);
          },
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              new Date(params.row.taskInstanceEnabledDateTime).format(
                "yyyy-MM-dd hh:mm:ss"
              )
            );
          },
        ...
        // },
        {
          ...
          render: (h, params) => {
            if (params.row.taskInstanceDateTimeLimit != -1) {
              var date = new Date(params.row.taskInstanceDateTimeLimit).format(
                "yyyy-MM-dd hh:mm:ss"
              );
            } else {
              date = "";
            }
            if (params.row.alertDeadline)
              return h("div", { style: { color: "red" } }, date);
        ...
        },
        {
          ...
          render: (h, params) => {
            return h(
              "div",
              params.row.taskInstanceStatus == 1 ? "" : params.row.submitterName
            );
          },
          sortable: true,
        },
  ...
  methods: {
    ...
    getTaskTotal() {
      ...
      var groups = that.store.state.user.userGroups.map((g) => g.name);
    ...
    async receiveTask() {
      ...
      await receiveTaskInstance(proId, taskId, userId, userDisplayName).then(
        (res) => {
          if (res.success) {
            // console.log("that.taskList.filter(x => x.id == item.id)[0]",that.taskList.filter(x => x.id == item.id)[0]);
            that.taskList.filter(
              (x) => x.id == item.id
            )[0].taskInstanceStatus = 2;
            that.taskList.filter(
              (x) => x.id == item.id
            )[0].taskInstanceStartDateTime = Date.parse(new Date());
    ...
    async getBindEnObjSingle() {
      let item = this.currTask;
      var obj = {};
      await getEobjSingle(item.bindEnClassName, item.enClassInstanceId).then(
        (res) => {
