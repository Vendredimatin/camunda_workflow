...
export const parseEscapeString = (oriString, env, user, obj, attributes, param, form) => {
  ...
  let intType = ['Integer', 'Long', 'Double'];
  ...
  _matches.forEach(async (x) => {
    ...
    switch (sqlList[0]) {
      case "$user":
        ...
        attr = sqlList[1] === 'userGroups' ? JSON.parse(sessionStorage.getItem(sqlList[1])) : sessionStorage.getItem(sqlList[1]);
        //app端分享
        if(sqlList[1] === 'token' && !attr && sessionStorage.getItem('shareAboutToken')){
          attr = sessionStorage.getItem('shareAboutToken')
        }
        if (attr == null) {
          _replaceMat = "";
        } else {
      ...
      case "$obj":
      {
        ...
        if (obj == null || sqlList.length == 1 || obj[sqlList[1]] == null) {
          if(sqlList[1]){
            ...
            _replaceMat = intType.includes(attr.valueType) ? null : "";
          }
        } else {
          let removePrefixAttr = relationExg.test(sqlList[1]) ? sqlList[1].replace(relationExg, '') : sqlList[1]; //去除关联类前缀之后的属性key
