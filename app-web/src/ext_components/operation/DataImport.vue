<template>
    <div>
        ...
        <Button style="display:inline-block; margin: 0 5px;" :type="args.type" :icon="args.icon" :disabled="args.disabled" :style="{'color': args.disabled ? '#bbbec4' : args.main_fontColor, 'height': arg_height}" :shape="args.shape ? 'circle' : null" @click="exportModal" id="exportTemplateButton">导出模板</Button>
        <Button style="display:inline-block; margin: 0 5px;" :type="args.type" :icon="args.icon" :disabled="args.disabled" :style="{'color': args.disabled ? '#bbbec4' : args.main_fontColor, 'height': arg_height}" :shape="args.shape ? 'circle' : null" @click="selectFile" id="importDataButton">导入数据</Button>
...
export default {
    ...
    methods: {
        ...
        async exportModal(){
            ...
            this.checkAll = false;
            if(Object.keys(store.state.DWF_form.Relations)[0] != null){
                this.AllChosedTargetClassName = Object.keys(store.state.DWF_form.Relations)[0];
                this.getClassNameValue();
            }else if(Object.keys(store.state.DWF_form.Entities)[0] != null){
                this.AllChosedTargetClassName = Object.keys(store.state.DWF_form.Entities)[0];
                this.getClassNameValue();
            }
