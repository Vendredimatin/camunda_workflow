      ...
      <span :style="{'height': arg_height, 'width': mainWidth, 'display': 'inline-block',
            ...
            <span v-show="!args.hided"
                ...
                <Select v-if="!needPop"
                        ...
                        clearable>
                    <Option v-for="(item,index) in args.selectList" :value="item.value" :key="index" class="self-color">{{ item.label }}</Option>
                </Select>
      ...
      <span :style="{'height': arg_height, 'width': '100%', 'display': 'inline-block',
            ...
            <span v-show="!args.hided"
                ...
                <Select v-if="!needPop"
                        ...
                        :style="{ 'width': '100%', 'display': 'inline-block', 'text-align': mainXAlign, 'vertical-align': mainYAlign, 'border': border, 'color': args_txt_fontColor, 'font-size': args_fsize}"
                        @on-change="handleValueChanged" clearable>
                    <Option v-for="item in args.selectList" :value="item.value" :key="item.value" class="self-color">{{ item.label }}</Option>
                </Select>
  ...
  export default {
    ...
    methods: {
      ...
      async initDOptions(className, attrName) {
        ...
        this.returnSelectFormat = attrName;
      ...
      getValue() {
        ...
        } else if ((this.value !== null && this.value !== '' && this.value !== undefined) || (Object.prototype.toString.call(this.value) === '[object Array]' && this.value.length > 0)) {
          temValue = this.value;
          const dType = this.args.targetDataType;
          if (dType == 'Integer' || dType == 'Long') {
            temValue = parseInt(temValue);
    ...
    setToolTip(mes, type, direction, width) {
      ...
      async remoteMethod(query) {
        ...
        if (query && query.trim() !== '') {
          ...
          queryContent = `cast(obj.${this.returnSelectFormat} as text) like '%${query || ''}%'`;
      ...
      async initBrowseListData(){
        let res = await getDistinctValuesWithParams(this.searchClass, this.returnSelectFormat, {
