<template>
    <section :addinName="name" ref="main" v-show="!args.hided" :style="{'width': arg_width}">
        ...
        <div class="echart-bar" ref='previewbox' id="ebox" style="width: 100%;" :style="{ 'height': arg_height}"></div>
        <!-- 导出数据前的预览 -->
        <!-- <Modal
...
export default {
  ...
  methods: {
    ...
    render() {
      ...
      let selfAreaColor = '#eee';
      let selfAreaBorderColor = 'rgba(0, 0, 0, 0.2)';
      let selfAreaLabelColor = 'rgba(0,0,0,0.4)';

      if(targetSkin == 'dark' && location.href.indexOf('displayType=') == -1) {

        selfAreaColor = '#254065';
        selfAreaBorderColor = '#00d5ff';
        selfAreaLabelColor = '#fff';
      ...
      this.args.option.geo = {
            ...
            label: {
              normal: {
                show: true, // 是否显示对应地名
                textStyle: {
                  color: selfAreaLabelColor
                }
              }
            },
            itemStyle: {
              normal: {
                areaColor: selfAreaColor,
                borderColor: selfAreaBorderColor
              },
              emphasis: {
                areaColor: null,
                shadowOffsetX: 0,
        ...
        this.mychart.on('click', function(params) {
            ...
            if(that.clickFlag) {
              that.invokeOperation(that.clickFlag.opr_type, that.clickFlag.opr_path, that.itemValue, that.store, null, that.clickData);
            } else {
              that.invokeOperation('', '', that.itemValue, that.store, null, that.clickData);
            }
