<template>
  <section v-show="!args.hided" id="inputDiv" :addinName="name"  dropTarget="0" ref="main" :style="{'width': colWidth}">
    ...
    <span :style="{'width': mainWidth, 'display': 'inline-block',
            ...
            <span v-show="!args.hided" :style="{'background-color': args.main_color, 'width': '100%', 'display': 'inline-block', 'vertical-align': mainYAlign, 'border': border}">
                <div>
                    <Select ref="sos"
                        ...
                        <Option v-for="(item, index) in browseList" :value="item.value" :key="index" :label="item.label" @click.native="handleSelectedClick(item.object)">
                    ...
                    <div v-if="!needPop">
                      <Dropdown
                        ...
                        <Input
                                ...
                                placeholder="请选择"
                                clearable
                                :style="{'height': arg_height, 'color': args_txt_fontColor, 'font-size': args_fsize, 'background-color': args.txt_bgColor}"
                                :disabled="args.readonly || t_visit"
                                @mouseenter.native="hoverEvent"
...
<script>
  ...
  import { getQueryOperation, getEobj, getEJobj,
    getEobjCount,
    getRelationObj,
  ...
  export default {
    ...
    props: ['itemValue', 'store', 'attributes'],
    ...
    computed: {
      ...
      dropdownClass(){
        return sessionStorage.getItem('skinStyle') === 'dark' && sessionStorage.getItem('appMenu') ? `self-color dark single-object-selector uuid_${this.dropdownUUID}` : `single-object-selector uuid_${this.dropdownUUID}`;
      },
      selectedObjectTitle(){
    ...
    methods: {
      ...
      async freshData(query){
        ...
        if(this.args.bindTargetClass.split('\&')[1] === 'r'){
          getRelation(this.args.bindTargetClass.split('\&')[0]).then(response => {
            if(response.success) {
        ...
        if(id !== this.args.id){
          ...
          for(let object of objects){
            ...
            if(objects.length > 1){
              ...
              if(this.args.returnFormat[0] in object){
                this.selectedObject = this.args.returnFormat.length !== 0 ?
                  (this.args.returnFormat.map(attr => {
                    return object[attr];
                  }).join("-"))
      ...
      setInheritStyle() {
        ...
        try {
          ...
          if(this.remoted){
            ...
            this.$refs.main.querySelector(".i-input .ivu-input").style.textAlign = 'center';
      ...
      resetQueryBox(){
        this.$refs.sos.clearSingleSelect();
      },
