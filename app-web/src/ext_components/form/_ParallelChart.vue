...
export default {
  ...
  methods: {
    ...
    render(){
      ...
      console.log(this.args.option, this.args.paralleAttr, this.args.valueAttribute);
      this.args.option.parallelAxis = [];
        for (let i = 0; i < this.args.valueAttribute.length; i++) {
            let attr = this.args.paralleAttr.filter(x => x.attributeName === this.args.valueAttribute[i])[0];
            this.args.option.parallelAxis.push({
                dim: i,
                ...
                type: attr.valueType === "String" ? "category" : "value",
            });
        }
        ...
        if(this.clickFlag) {
            this.myChart.on('click', function(params) {
                ...
                that.invokeOperation(that.clickFlag.opr_type, that.clickFlag.opr_path, that.itemValue, that.store);
            })
        }
        // 内部脚本调用
        if(this.args.echartScript != '') {
