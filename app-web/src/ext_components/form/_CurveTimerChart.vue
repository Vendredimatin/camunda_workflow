...
export default {
  ...
  methods: {
    ...
    render(){
        this.args.option.xAxis = { name: this.args.xAxisname, type: "value", axisLine: {onZero: false}, axisLabel:{show:false} };
        ...
        var timer = setInterval(() => {
            let start = repeat * that.args.dataStep;
            let end = start + that.args.maxNum;
            that.args.option.series[0].data = (() => {
                var res = [];
                that.args.option.dataset.source.slice(1).slice(start, end).forEach(x => (res.push(
                    (() => {
                        let res = [];
                        res.push(x[that.nameIndex]*4);
                        res.push(x[that.valueIndex]);
                        return res;
                    })()
                    )));
                return res;
            })();
            that.myChart.setOption(that.args.option);
            console.log(repeat);
            if (repeat >= that.args.timerLimit-1) {
                repeat = 0;
            } else {
                repeat++;
            }
