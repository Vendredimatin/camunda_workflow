    ...
    <section v-show="!args.hided" :addinName="name" id="inputDiv" ref="main" :style="{'width': colWidth}">
      ...
      'text-align': labelXAlign, 'vertical-align': labelYAlign, 'padding-right': '10px'}">
        ...
        <span :style="{'background-color': args.main_color, 'width': '100%', 'display': 'inline-block', 'vertical-align': mainYAlign}">
          ...
          >
          <Icon :style="{'color': args_fcolor}" v-if="t_create || t_edit" :type="args.suffixIcon" slot="suffix" @click="exportCode('t_edit')"/>
          <Icon :style="{'color': args_fcolor}" v-else :type="args.suffixIcon" slot="suffix"/>
          </Input>
          <div v-else>
            <Poptip v-show="hoverPop" trigger="hover" :placement="popHoverDirection" :width="popHoverObj.popWidth">
              <Input
                      class="i-input self-color"
                      v-model="value"
                      :placeholder="args.placeholder"
            ...
            <Poptip v-show="!hoverPop" trigger="focus" :placement="popFocusDirection" :width="popFocusObj.popWidth">
              <Input
                      class="i-input self-color"
                      v-model="parseValue"
                      :placeholder="args.placeholder"
...
export default {
  ...
  created() {
    // 从store中获取查询到的数据结果
    let uuid = this.QueryResult({query: this.query, targetClass: this.itemValue.data.targetClass});
    if (uuid) uuid = uuid[0];
    // 给编码$obj取值用，相当与visit的this.itemValue.data.origin
    this.rootJsonOrigin = ( this.itemValue.data.isRelation ? this.RelationSingleA(this.itemValue.data.targetClass, uuid) : this.EntitySingle(this.itemValue.data.targetClass, uuid));
    //通过prop给控件初始化
  ...
  mounted() {
    ...
    // });
        ...
        computed: {
            ...mapGetters("DWF_form", ["EntitySingle", "RelationSingleA", "QueryResult"]),
            arg_height() {
  ...
  methods: {
    ...
    exportCode(flag) {
      ...
      let finalParseStr = this.parseEscapeString(this.args.encodePrefix, null, null, this.itemValue.data.origin, this.attributes, this.store);
      let changString = finalParseStr + dateStream
      // let finalParseStr = this.parseEscapeString(this.args.encodePrefix, null, null, this.rootJsonOrigin, this.attributes, this.store);
      if(this.value){
        this.parseValue = this.value
      }
      if(this.t_create || (this.t_edit && flag == 't_edit') || flag == 'all'){
        this.parseValue = changString
        this.value = changString
        flag !== 'setValue' ? this.valueChange(this.value) : ''
      }
    },
    // 动态监听数据变化
    onDynamic(value) {
