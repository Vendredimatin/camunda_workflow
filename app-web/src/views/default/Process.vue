<template>
    <div class="process" style="height: 100%;">
        ...
        <div class="process-Button" style="width: 100%;padding:0px 20px;">
            <Button class="self-btn" type="primary" @click="saveForm" :disabled="editAuthority != 2">保存</Button>
            <Button class="self-btn" type="primary" @click="openSubmitModal" :disabled="editAuthority != 2">提交</Button>
            <Button class="self-btn" type="primary" @click="openAssignModal" :disabled="editAuthority != 2">移交</Button>
            <div style="float:right;">
                <Button class="self-btn" type="error" @click="deleteInstMadel = true" :disabled="editAuthority != 2">终止</Button>&nbsp;
                <Button class="self-btn" size="large" icon="ios-chatbubbles-outline" type="primary" shape="circle" :disabled="editAuthority == 1" @click="commentDrawer = true"  ></Button>
        ...
        <wf-process-comments
            ...
            :pid="route.meta.wfProcessInstanceId" :tid="route.meta.taskId"
        ...
        <Modal v-model="assignModal" :mask-closable="false" title="选择用户">
            ...
            <div slot="footer">
                <Button class="self-btn" type="text" size="large"  @click="closeAssignMoal">取消</Button>
                <Button class="self-btn" type="primary" size="large" @click="onConfirmAssign">确认</Button>
            </div>
...
<script>
    ...
    import {deleteProcessInstance, sendCommand, launchProcessByObjId} from "@/api/wfprocess"
    export default {
        name: 'process',
        ...
        computed: {
            ...
            editAuthority(){
                ...
                if(this.route.meta.wfAuthority == 'history'||this.route.meta.wfAuthority == 'instance') return 4;
                if(this.route.meta.wfAuthority == "check") return 1;
            }
        },
        methods: {
            async init(){
                await getUser(this.store.state.user.userId).then(res=>{ this.userDisplayName =  res.data.data.displayName;});
            },
            activate() {
                ...
                if(user.userId == ''||user.userId == null){
                    ...
                    sendCommand(data).then(re=>{
                        if(re.success){
                            ...
                            setTimeout(() => {that.root.onTabRemove(that.route.name);}, 500);
