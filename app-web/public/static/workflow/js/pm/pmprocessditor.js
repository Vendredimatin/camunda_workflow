...
(function ($, window, document, undefined) {
  ...
  var Editor = function (element, options) {
    console.log("Editor",Editor, service);
    ...
    console.log("wfProcessEditor this.options",this.options);
    this._defaults = defaults;
    this._name = pluginName;
    this.stack = new CommandStack(this.options.pid);
  ...
  Editor.prototype.loading = function (options = this.options) {
    var that = this;
    ...
    ajaxGet(service.buildAPI("/process-template/"+options.id)).complete(function (res) {
      var data = res.responseJSON;
      console.log("api6data",data);
      if (data.success) {
        that.wfprocess.parseFromJSON(JSON.parse(data.data));
        console.log("that.wfprocess",that.wfprocess);

        that.wfprocess.setContext(that.context);
        that.repaint();
        that.setPropertySheet();
        that.options.proinfo.setPropertySheet.$Message.info("流程模版加载完成");
      } else {
        that.options.proinfo.setPropertySheet.$Message.info("获取流程模版失败");
      }
    });
  };
  ...
  Editor.prototype.createToolbar = function (options) {
    ...
    var c = "btn btn-default self-btn";
    var c1 = "btn btn-dangerous self-btn";
    ...
    this.svbutton = this.createTool(g6, "savebutton" + this.options.id,
      "保存", c, "i", "fa fa-check-square-o fa-lg");
    this.svbutton.classList.add("savebutton");
    this.disableButton(this.svbutton);

    var g7 = this.createGroup(parent);
    this.refreshbutton = this.createTool(g7, "refresh" + this.options.id, "刷新",
      c, "i", "fa fa-refresh fa-lg");

    this.stack.undoButton = this.undobutton;
    this.stack.redoButton = this.redobutton;
    this.stack.refreshbutton = this.refreshbutton;
  ...
  Editor.prototype.component = function (title, id, iconname) {
    ...
    com.className = "list-group-item self-panel";
  ...
  Editor.prototype.createPalletBar = function (id, barname, expanded, clpin,
    ...
    panelHeading.className = "panel-heading self-panel";
  ...
  Editor.prototype.initCanvas = function (options) {
    ...
    canvasPanel.style.width = "calc(100% - 76px)";
    // canvasPanel.style.width =  "90%";
    ...
    this.canvasPane.id = "convasPane_" + options.id;
    ...
    this.canvas.id = 'painterconvas_' + options.id;
  ...
  Editor.prototype.doClick = function (evt) {
    if (evt.target == this.undobutton
      ...
      //     console.log("13");
    } else if (evt.target == this.svbutton
      || evt.target.id == ("savebutton" + this.options.id)) {
      console.log("pmprocessditor savebutton");
      this.options.parent.doSaveAction();
    } else if (evt.target == this.pbutton
      || evt.target.id == ("publish" + this.options.id)) {

      this.options.proinfo.setPropertySheet.releaseWfProcess();
  ...
  Editor.prototype.setPropertySheet = function () {
    ...
    // console.log("pmpro--obj",obj);
    // console.log("this.options",this.options)
    if(this.options.proinfo.setPropertySheet!=null){
      this.options.proinfo.setPropertySheet.setPropertySheet(obj);
      if(obj instanceof  WfProcess){
        this.options.proinfo.setPropertySheet.setWfProcess(obj);
      }
    }
  ...
  Editor.prototype.doMouseDown = function (evt) {
    ...
    // console.log("doMouseDown this",this,map);
    evt.preventDefault();
    var mouse = this.getPointOnCanvas(evt);
    $('.nav-tabs a[href="#tab_' + this.options.id + '"]').tab('show');
    this.mousepressed = true;
    this.mouse0 = this.getPointOnCanvas(evt);
    // console.log("doMouseDown",this.wfprocess instanceof WfProcess);
    let tt = this.wfprocess.__proto__;
    while (tt != null) {
      // console.log("tt:", tt);
      tt = tt.__proto__;
    }
    ...
    // console.log("doMouseDown2",this.wfprocess instanceof WfProcess);
  ...
  Editor.prototype.doMouseUp = function (evt) {
    // console.log("Editor.prototype.doMouseUp");
    evt.preventDefault();
    this.mousepressed = false;
    // console.log("doMouseUp",this.wfprocess instanceof WfProcess);
    if (this.editorStatus == 0) {
  ...
  Editor.prototype.doCreationAction = function (obj) {
    var that = this;
    if (obj != null) {
      ajaxGet(service.buildAPI("/getNewID")).complete(function (res) {
        var data = res.responseJSON;
        if(data.success){
          obj.id = data.data;
          // 自动绑定可访问变量
          if(obj instanceof ManualTask || obj instanceof StartPoint||obj instanceof EndPoint){
            var avList = [];
            var vars = that.wfprocess.listVariables();
            if (vars != null && vars.length > 0) {
              for (var i = 0; i < vars.length; i++) {
                ...
                a.varId = vars[i].id;
                a.accessControl = 1;
                a.currOwner = obj.currOwner;
                a.owner = obj.owner;
                a.componentId = vars[i].name;
                a.description = vars[i].description;
                avList.push(a);
              }
              obj.accessibleVars=avList;
              console.log(">>obj",obj);
            }
          }
          // 自动加办理人
          if(obj instanceof ManualTask){
            var par = [{'participantType':'launcher','name':'','oid':''}];
            obj.participants = par;
          }
          that.addtoGraph(obj);
        }
      });
